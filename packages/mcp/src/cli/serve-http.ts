...
import { SupabaseOffsetStore } from "../persistence/supabase.js";
...
export async function serveHttp(): Promise<void> {
  ...
  // Single shared TelegramTransport — init once, reuse across requests
  ...
  const httpServer = createServer(async (req: IncomingMessage, res: ServerResponse) => {
    try {
      ...
      if (url.pathname === "/mcp" && req.method === "POST") {
        ...
        // Attach parsed body to req so the transport can access it
