...
(SHOULD_RUN ? describe : describe.skip)("PgvectorStore (live pg)", () => {
  ...
  it("filters search by workspaceId", async () => {
    ...
    const ws1Source = (await client.query(`INSERT INTO knowledge_sources_test (workspace_id) VALUES ($1) RETURNING id`, [ws1])).rows[0].id;
    const ws2Source = (await client.query(`INSERT INTO knowledge_sources_test (workspace_id) VALUES ($1) RETURNING id`, [ws2])).rows[0].id;
    const f1 = (await client.query(`INSERT INTO knowledge_files_test (source_id, path) VALUES ($1, 'a') RETURNING id`, [ws1Source])).rows[0].id;
    const f2 = (await client.query(`INSERT INTO knowledge_files_test (source_id, path) VALUES ($1, 'b') RETURNING id`, [ws2Source])).rows[0].id;
    await store.upsert(f1, [{ fileId: f1, chunkIndex: 0, text: "ws1", embedding: [1,0,0,0], tokenCount: 1, metadata: {} }]);
    await store.upsert(f2, [{ fileId: f2, chunkIndex: 0, text: "ws2", embedding: [1,0,0,0], tokenCount: 1, metadata: {} }]);
