...
describe("handleEmailPollFallback", () => {
  ...
  it("continues on per-token failure", async () => {
    const tokens = [
      { id: "t1", email_address: "a@a.com", last_history_id: "10", status: "active", refresh_token_encrypted: "e", workspace_id: "x", channel_id: "x", scopes: [], watch_expiration: null, last_error: null, consecutive_renewal_failures: 0, created_at: "x", updated_at: "x" },
      { id: "t2", email_address: "b@b.com", last_history_id: "20", status: "active", refresh_token_encrypted: "e", workspace_id: "x", channel_id: "x", scopes: [], watch_expiration: null, last_error: null, consecutive_renewal_failures: 0, created_at: "x", updated_at: "x" },
    ];
    const tokenStore = { list: vi.fn(async () => tokens) };
    const fetchOne = vi.fn()
