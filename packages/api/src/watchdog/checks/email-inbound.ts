...
export async function checkEmailInbound(deps: EmailInboundCheckDeps): Promise<CheckResult> {
  ...
  if (tokens.length === 0) {
    return {
      id: ID,
      status: "down",
      message: "email sin token configurado",
      action: deps.reauthUrl,
    };
  }
  const tok = tokens[0] as EmailToken;
  if (tok.status !== "active") {
    const suffix = tok.last_error ? ` (${tok.last_error})` : "";
    return {
      id: ID,
      status: "down",
      message: `email status=${tok.status}${suffix}`,
      action: deps.reauthUrl,
    };
  }
  if (tok.consecutive_renewal_failures >= 1) {
    return {
      id: ID,
      status: "down",
      message: `email: ${tok.consecutive_renewal_failures} fallo(s) de renovación del watch`,
      action: deps.reauthUrl,
    };
  }
  if (tok.watch_expiration) {
    ...
    if (expMs < nowMs + deps.expiryMarginMs) {
      ...
      return {
        id: ID,
        status: "down",
        message: `email: watch expira en ~${hours}h`,
        action: deps.reauthUrl,
      };
    }
  }
